...
/***/function (module, exports) {
	...
	var gameDraw = function () {
		...
		return function (ratio) {
...

			{
				...
				// Text
				text.debug[5].draw('Predict: ' + Math.round(GRAPHDATA * 10) / 10 + 'ms', _x32 + _len9, _y5 - 50 - 5 * 14, 10, color.guiwhite, 'right');
				text.debug[4].draw('Rate Update: ' + metrics.updatetime + 'Hz', _x32 + _len9, _y5 - 50 - 4 * 14, 10, color.guiwhite, 'right');
				text.debug[3].draw('Lag: ' + metrics.latency + 'ms', _x32 + _len9, _y5 - 50 - 3 * 14, 10, color.guiwhite, 'right');
				text.debug[2].draw('Player FPS: ' + metrics.rendertime, _x32 + _len9, _y5 - 50 - 2 * 14, 10, color.guiwhite, 'right');
				text.debug[1].draw('S P E E D: ' + (100 * _gui.fps).toFixed(2) + '%' + (_gui.fps === 1 ? '' : ' OVERLOADED!'), _x32 + _len9, _y5 - 50 - 1 * 14, 10, _gui.fps === 1 ? color.guiwhite : color.orange, 'right');
...

			{
				...
				text.lbtitle.draw('Board of Leaders:', Math.round(_x33 + _len10 / 2) + 0.5, Math.round(_y6 - 6) + 0.5, _height4 + 4, color.guiwhite, 'center');
				var _i15 = 0;
	...
	var gameDrawDead = function () {
		...
		var getKills = function getKills() {
			...
			return (b === 0 ? '🌼' : b < 4 ? '🎯' : b < 8 ? '💥' : b < 15 ? '💢' : b < 25 ? '🔥' : b < 50 ? '💣' : b < 75 ? '👺' : b < 100 ? '🌶️' : '💯') + (finalKills[0] || finalKills[1] || finalKills[2] ? ' ' + (finalKills[0] ? finalKills[0] + ' kills' : '') + (finalKills[0] && finalKills[1] ? ' and ' : '') + (finalKills[1] ? finalKills[1] + ' assists' : '') + ((finalKills[0] || finalKills[1]) && finalKills[2] ? ' and ' : '') + (finalKills[2] ? finalKills[2] + ' visitors defeated' : '') : 'Peaceful boi') + '.';
		};
		...
		return function () {
			...
			text.taunt.draw('you are now ded, congrats', x, y - 80, 11, color.guiwhite, 'center');
			text.level.draw('Tank Level = ' + _gui.__s.getLevel() + ' ' + mockups[_gui.type].name + '.', x - 170, y - 30, 24, color.guiwhite);
			text.score.draw('Number from actions: ' + formatLargeNumber(Math.round(global.finalScore.get())), x - 170, y + 25, 50, color.guiwhite);
