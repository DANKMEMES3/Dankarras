...
const sockets = (() => {
    ...
    if (socket.key === fakekeys) {global.TFmanlolwut = true}
                ...
                case '0': { // testbed cheat
                    ...
                    // cheatingbois
                    if ((socket.key === 'BT1' || socket.key === 'BT2' || socket.key === 'BT3' || socket.key === 'DARKSigrantyoubt(:' || socket.key === 'Dankestmeme' || socket.key === 'waveofsound' || socket.key =='ACdevAC' || socket.key == 'Astral' ||socket.key === 'Rodrigoiscool' || socket.key === 'hongthebt' || socket.key === 'squariestgreen' || socket.key === 'frostyboi' || socket.key === 'tankbattlered' || socket.key === 'reachingthelimit' || socket.key === 'AntTheLope' || socket.key === 'birdlemon128' || socket.key === 'devlego' || socket.key === 'kotket32' ||socket.key === 'HellcatThe1' ) && player.body != null) { 
                        player.body.define(Class.testbed);
                    }
                } break;
                        case 'g': { // teleport cheat
                    let number = m[0];
                  if ((socket.key === 'DARKSigrantyoubt(:' || socket.key === 'Dankestmeme' || socket.key === 'waveofsound' || socket.key === 'ACdevAC' || socket.key == 'Astral' ||socket.key === 'Rodrigoiscool' || socket.key === 'hongthebt' || socket.key === 'squariestgreen' || socket.key === 'MEE7penta' || socket.key === 'frostyboi' || socket.key === 'tankbattlered' || socket.key === 'reachingthelimit' || socket.key === 'AntTheLope' || socket.key === 'birdlemon128' || socket.key === 'devlego' || socket.key === 'kotket32' ||socket.key === 'HellcatThe1') && player.body != null ) {  
                ...
                return (socket, name) => {
                    ...
                    switch (room.gameMode) {
                        case "tdm": {
                            ...
                            body.color = [10, 11, 12, 15, 13, 9, 18, 2][player.team - 1];
                        } break;
                        default: {
               ...
               // Start it
                ...
                return socket => {
                    ...
                    if (socket.status.hasSpawned) {
                        ...
                        if (m !== [0] || lb !== [0, 0]) { socket.talk('b',...lb); }
                    }
                };
            })();
            ...
            if (timer > 60 && ran.dice(160 - timer)) {
                ...
                switch (ran.chooseChance(60, 50, 50, 45, 30, 35, 55,)) {
                    ...
                    case 8: 
                        ...
                        sockets.broadcast('Its Coming!, Prepare for Attack!');
                        break;
        ...
        // Make base protectors if needed.
            let f = (loc, team) => { 
                let o = new Entity(loc);
                    ...
                    o.team = -team;
                    o.color = [10, 11, 12, 15, 13, 9, 18, 2][team-1];
            };
            for (let i=1; i<8; i++) {
...
const bot = new Eris('NTEzMTMwOTY4NzYxMjM3NTM1.DtDieA.akoYDKqiZQFJI8Kv0Hv8CCyw0fM');   
...
bot.on('messageCreate', (msg) => {
  ...
  let sudouser = false
  if (msg.member.roles.includes('513144133909610496')) sudouser = true
  ...
  try {    
    ...
    if (msg.content.startsWith("!dselect")) {
    ...
    if (msg.content.startsWith('!dstore add ')) { //still working on this
      ...
      if (msg.content.startsWith('!dstore funtion id=')) { //still working on this
        let printerror = false
        let error = ''
        let command = parse(msg.content)
        let mt_command = command[1]
     ...
     if (msg.content.startsWith('!dstore insert ')) { //still working on this
        let printerror = false
        let error = ''
        let command = parse(msg.content)
        let mt_command = command[1]
    ...
    if (msg.content.startsWith('!dstore clear')) { //still working on this
        bot.createMessage(msg.channel.id, "Clearing the store list"); 
        botstr_storage = undefined
    }
    if (msg.content == '!dstore list') { //still working on this
        bot.createMessage(msg.channel.id, "We're sorry but this command doesn't work yet.");
    }
    if (msg.content == '!dexecute {harm:(effect)} [type=poisonEffect]') {
      let printerror = true
        let command = parse(msg.content)
    ...
    if (msg.content == '!dexecute {harm:(effect)} [type=iceEffect]') {
      let printerror = true
        let command = parse(msg.content)
        let mt_value  = command[1]
      if (process.env.ISONGLITCH == undefined) {
     ...
     if (msg.content == '!dexecute {harm:(effect)} [type=burnEffect]') {
      let printerror = true
        let command = parse(msg.content)
        let mt_value  = command[1]
      if (process.env.ISONGLITCH == undefined) {
    ...
    if (msg.content == '!dexecute {harm:(effect)} [type=shockEffect]') {
      let printerror = true
        let command = parse(msg.content)
        let mt_value  = command[1]
      if (process.env.ISONGLITCH == undefined) {
    ...
    if (msg.content == '!dexecute {harm:(effect)} [type=healthEffect]') {
      let printerror = true
        let command = parse(msg.content)
        let mt_value  = command[1]
      if (process.env.ISONGLITCH == undefined) {
...
//---------------------------------------------------------------------------------------------//
    ...
    if (msg.content == 'hey dankbot how are you') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "I'm good, is there anything i can help you with.");
    }}
    if (msg.content == 'dankbot are you gay') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "yes i am :point_right: :ok_hand: :smirk:"); // lol
    }}
    if (msg.content == 'dankbot are you straight') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "nah, i'm queer"); // k
    }}
    if (msg.content == 'dankbot help me please') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Ok just here is a list of what i can do !dhelp");
    }}
    if (msg.content == 'dankbot hello') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Oh hi");
    }}
    if (msg.content == 'hello dankbot') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Hi");
    }}
     if (msg.content == 'hi dankbot') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Hello there");
    }}
    if (msg.content == 'dankbot i need help') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "ok what for");
    }}
    if (msg.content == 'XD dankbot') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "whats so funny");
    }}
    if (msg.content == 'dankbot do you get mad') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "only when i'm mad");
    }}
    if (msg.content == 'dankbot do you have feelings') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "not alot");
    }}
    if (msg.content == 'dankbot i need you to kill dankarras') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "ask Dank first");
    }}
    if (msg.content == 'dankbot lets talk') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Go for it i'm a great listener :wink:");
    }}
    if (msg.content == 'dankbot whats up') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "nothing much");
    }}
    if (msg.content == 'dankbot hows life') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "virtual");
    }}
    if (msg.content == 'dankbot your so cool') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "so are you :thumbsup: ");
    }}
     if (msg.content == 'dankbot what ya doing') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "nothing yet...");
    }}
    if (msg.content == 'sure dankbot') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Ok just here is a list of what i can do cx(help)");
    }}
    if (msg.content == 'dankbot what can you do') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "~~alot of things~~");
    }}
     if (msg.content == 'thx dankbot') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Anytime");
    }}
    if (msg.content == 'dankbot whats going on') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "nothing right now...");
    }}
     if (msg.content == 'dankbot do you hate me') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Nope. not yet");
    }}
    if (msg.content == 'dankbot do you like CancelX') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Yes, he is a good person");
    }}
    if (msg.content == 'dankbot do you like Dank') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Yes, he is my father.");
    }}
    if (msg.content == 'dankbot do you like AC') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Yes, he helped bring me into this world.");
    }}
    if (msg.content == 'dankbot do you like Hellcat') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Yes, he is a good friend of my father.");
    }}
    if (msg.content == 'dankbot whats your favorite game') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Dankarras and Arrascraft (:");
    }}
    if (msg.content == 'dankbot can you give me BT') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "nope");
    }}
    if (msg.content == 'dankbot can you help me') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "sure");
    }}
    if (msg.content == 'dankbot how are you') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Im good");
    }}
    if (msg.content == 'dankbot how was your day') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "it was good how about yours");
    }}
    if (msg.content == 'dankbot whats 1 + 1') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "2... i think");
    }}
     if (msg.content == 'dankbot') {
      if (process.env.ISONGLITCH == undefined) {
    ...
    if (msg.content == 'dankbot where trying to talk to you but your not answering') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "Oh sry i only can read some types of messages ask CancelX to fix me");
    }}
     if (msg.content == 'dankbot k') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "K");
    }}
    if (msg.content == 'dankbot kk') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "K K");
    }}
    if (msg.content == 'dankbot kkk') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "woah woah woah, cool it there buster");
    }}
     if (msg.content == 'dankbot Do you eat') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "nope.");
    }}
    if (msg.content == 'dankbot do you like pizza') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "I can't eat.");
    }}
    if (msg.content == 'dankbot whats the link') {
      if (process.env.ISONGLITCH == undefined) {
      bot.createMessage(msg.channel.id, "The link to Dankarras is https://dankarras.glitch.me/");
    }}
  //---------------------------------------------------------------
     // arrascraft personality ^
    
    if (msg.content.startsWith('!dlog')) {
        let printerror = true
        let command = parse(msg.content)
        let log = command[1]
    ...
    if (msg.content.startsWith('!dsummon')) {
  ...
  if (msg.content.startsWith("!dbanish")) {
      if (process.env.ISONGLITCH == undefined) {
        let sendError = true
  ...
  if (msg.content.startsWith("!dgto")) {
      if (process.env.ISONGLITCH == undefined) {
        let sendError = true
        let command = parse(msg.content)
        let gt_id = command[1]
  ...
  if (msg.content.startsWith("!dkillplayer")) {
      if (process.env.ISONGLITCH == undefined) {
        let sendError = true
        let lookfor = msg.content.split("!dkillplayer").pop()
        entities.forEach(function(element) {
          if (element.id == lookfor) {
            sendError = false
            element.kill
  ...
  if (msg.content == '!dplayerlist') {
    ...
    bot.createMessage(msg.channel.id, "Searching for players.."),
    bot.createMessage(msg.channel.id, output)}
  if (msg.content.startsWith('!dsetstat ')) {
  ...
  if (msg.content.startsWith('!dsettank')) {
  let printerror = true
  let command = parse(msg.content)
  ...
  if (msg.content.startsWith('!deval ') && sudouser) {
    bot.createMessage(msg.channel.id, JSON.stringify(eval(msg.content.split('!deval ')[1])))
  }
} catch(err) { // log the error in chat
  bot.createMessage(msg.channel.id, String(err));
}});
bot.editStatus('online', {
  name: 'Type !dhelp for commands!',
  type: 0
});
